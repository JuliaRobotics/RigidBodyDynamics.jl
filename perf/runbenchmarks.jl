...
function create_benchmark_suite()
    ...
    torques = SegmentedVector(Vector{ScalarType}(num_velocities(mechanism)), tree_joints(mechanism), num_velocities)
    ...
    suite["inverse_dynamics"] = @benchmarkable(begin
        ...
        v̇ = SegmentedVector(rand(ScalarType, num_velocities($mechanism)), tree_joints($mechanism), num_velocities)
    ...
    suite["dynamics"] = @benchmarkable(begin
        ...
        rand!($state)
        τ = SegmentedVector(rand(ScalarType, num_velocities($mechanism)), tree_joints($mechanism), num_velocities)
        externalwrenches = RigidBodyDynamics.BodyDict(RigidBodyDynamics.id(body) => rand(Wrench{ScalarType}, root_frame($mechanism)) for body in bodies($mechanism))
    end)
