...
@testset "spatial" begin
    ...
    @testset "show" begin
        ...
        show(DevNull, rand(Twist, f2, f1, f3))
        show(DevNull, rand(SpatialAcceleration, f2, f1, f3))
        show(DevNull, rand(Wrench, f2))
        show(DevNull, rand(Momentum, f2))
    ...
    @testset "twist" begin
        T1 = rand(Twist, f2, f1, f3)
        T2 = rand(Twist, f3, f2, f3)
        T3 = T1 + T2
        ...
        @test_throws ArgumentError T1 + rand(Twist, f3, f2, f4) # wrong frame
        @test_throws ArgumentError T1 + rand(Twist, f3, f4, f3) # wrong base
    ...
    @testset "momentum" begin
        T = rand(Twist, f2, f1, f2)
        I = rand(SpatialInertia{Float64}, f2)
        T2 = rand(Twist, f2, f1, f1)
        H21 = rand(Transform3D, f2, f1)
        h = I * T
    ...
    @testset "At_mul_B!" begin
        ...
        vec = rand(SpatialAcceleration, f2, f3, f1)
    ...
    @testset "spatial acceleration" begin
        I = rand(SpatialInertia{Float64}, f2)
        Ṫ = rand(SpatialAcceleration, f2, f1, f2)
        T = rand(Twist, f2, f1, f2)
    ...
    @testset "log / exp" begin
        ...
        ξ = Twist(f2, f1, f1, zeros(SVector{3}), rand(SVector{3}))
        H = exp(ξ)
        @test isapprox(ξ, log(H))
        ...
        for θ in linspace(1e-3, π - 1e-3, 100) # autodiff doesn't work close to the identity rotation
            ξ = Twist(f2, f1, f1, θ * normalize(rand(SVector{3})), θ * normalize(rand(SVector{3})))
            H = exp(ξ)
            T = Twist(f2, f1, f2, rand(SVector{3}), rand(SVector{3}))
